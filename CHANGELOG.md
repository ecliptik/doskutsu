...
- **Waves 36-37 -- production floor + ship-discipline machinery.** After the wave-38/39 failed-arc (see Performance section below), the production floor was hardened with revert-verification gates so future regressions can't re-ship the same broken patches. Patches: `patches/SDL/0055` (Phase 12 SDL-side audio sub-bucket brackets; default-OFF; surfaces cooperative-scheduler audio_thread as dominant audio cost), `0056` (runtime killswitch for SDL/0054 silence-detect via `SDL_HINT_DOSKUTSU_SILENCE_DETECT=0`), `0057` (Shape D mixer-flag silence-detect; reads SDL3_mixer `doskutsu_mixer_actual_mixed_bytes` O(1) instead of bytewise scanning), `0058` (Shape A mechanism histogram for cost attribution), `patches/SDL_mixer/0001` (mixer-flag setter for Shape D). Plus tooling: `tests/run-gameplay-smoke.sh` BANNERS array enforces banner-emit gate; `scripts/verify-patches-applied.sh` + `make patches` target ensures patch series re-applies cleanly; `make stage` creates `DOSKUTSU/` subdir for SDL log routing; wave-iter sparse-PLAY templates with explicit per-wave PLAY declarations.
- **Wave 35 -- Pixtone SFX correctness fix.** `patches/SDL/0054` (Shape A silence-detect): deletes the gate-of-buffer-size-0 trap in `DOSSOUNDBLASTER_GetDeviceBuf`, adds silence-skip-to-DSP-write logic in `PlayDevice`. Mixer always ticks so Pixtone fires reach the stream. Real-HW operator confirmed SFX audible across all 7 wave-35 PLAYs. Cost was ~30x prediction error -- spec said +0.5% CPU (~0.2 fps), measured -5.96 fps via `fi_ms` slice +3.34 ms regression (silence-detect per-iter mix-buffer scan overlaps framebuffer dosmemput slice). Companion patches: `nxengine-evo/0131` (Phase 12 audio instrumentation; default-OFF), `0132` (mds_tilemap_bg `drawTileFast` BG-extension via `_blit_indexed` INDEX8 fast-path; default-ON; killswitch `SDL_HINT_DOSKUTSU_DRAWTILE_FAST_BG=0`).
- **Wave 34 -- backdrop sub-region partial blit.** `patches/nxengine-evo/0130`: per-map FG-occlusion scan + per-frame visible-Y clip; new `drawCachedBackdropClipped()` Renderer method for HIT path; THRASHING tile loop outer-Y clipped to visible range. Predicted -1.5 to -3 ms / +1.5 to +3 fps; landed +3.21 fps (within band). First robust crossing of 40 fps. Bandwidth-bound mechanism class validated on PODP83+Cirrus. Known visual regression: black-backdrop at bottom on jump-induced foreground dips (FG-occlusion scan over-aggressive); operator marked not-priority.
- **Wave 33 -- H3 visit-loop bounds-check elimination + SDL/0053 freeze-on-quit fix.** `patches/nxengine-evo/0128` (template-factored loop; new `SDL_HINT_DOSKUTSU_TILE_BOUNDS_SKIP` default-ON), `0129` (SoundManager BeginShutdown companion), `patches/SDL/0053` (engine-signaled pre-shutdown gate disable; resolves SDL/0050 cooperative-scheduler-vs-`MIX_DestroyMixer` freeze). Wave-31 Lever B output cache patch (`0127`) moved to `patches/nxengine-evo/_disabled/` after measuring -2.75 fps regression (bandwidth-equivalent absorbed cache-blit-OUT); `apply-patches.sh` skips `_disabled/` subdir.
- **Phase 10 + Phase 11 wave 22-31 perf-decomposition + audio-investigation arc** (commit `aafca58` -- 38 patches across `patches/SDL/0036-0052` + `patches/nxengine-evo/0095-0127`). Highlights: keyboard buffer-overflow replace-not-chain, audio synth helpers (MPU-401 / OPL3 / idle pause), SB16 IRQ-5 instrumentation with RDTSC/PIT timing, DSP-mediated WaveBlaster MIDI exploration, Phase 10 force-pause + banner routing, MPU-401 probe defensive + skip-by-default, CloseDevice DSP hard-reset, `WaitDevice` shutdown-check unblocks `MIX_DestroyMixer`, `SDL/0050` DOS quiescence gate (+7 fps; ships default-ON with documented quit-freeze trade -- subsequently obsoleted by SDL/0053 in wave 33). Engine side: `0095-0096` mode_tick + mode_draw_scene decomp instrumentation, `0100-0110` Phase 10 audio (instrumentation, MIDI scheduler, WaveBlaster, OPL3 register programming, auto-detect chain, hanging-note fix), `0111` mds_tilemap_fg instrumentation, `0112` preload MIDI cache (eliminates first-play stall), `0113` tile bbox skip, `0117` iter K `drawTileFast` inline (+5 fps; default-ON), `0124` wave-28 flip() body decomposition, `0125` wave-29 mds_tilemap_bg decomposition, `0126` **wave 30 backdrop cache miss-streak direct-window (+4.13 fps; default-ON; largest single-iter delta in project history)**.
- **Build chain + tooling for Phase 10 + Phase 11**: probe-build chain extensions across `MEMBW`, MPU/SDL probes, `AUDBUF`, `IDLEPROBE`, `OPAQUE`, `BLTFILL`, `CHIPID`, `SDLPROBE` family. `scripts/fetch-cs-midi.py` + `build-cs-midi-orgmid.py` for hardware MIDI fetch (wiimidi + wiimidi2 from cavestory.one, plus Robert Hart's ORGMID converter for the one missing track white.org). `docs/ASSETS.md` updated with Phase 10 MIDI fetch documentation.
- **Wave 21 -- Lever G audio rate reduction.** New `SDL_HINT_DOSKUTSU_AUDIO_TIER2` env hint with strict-`0` killswitch semantics. Tier-2 = 11025 Hz mono with buffer pinned to 256 frames; halves the SB16 IRQ rate (~86 -> ~43 Hz) AND halves per-IRQ mixing CPU work. Three patches: `patches/nxengine-evo/0092` (rate config externs + 9 channel call sites in Pixtone + 1 in Organya substituted to runtime `AUDIO_CHANNELS`), `0093` (polarity flip -- Tier-2 becomes default; `=0` killswitch reverts to Tier-1), `0094` (Organya stereo->mono downmix at `_musicCallback` boundary; fixes the pre-0094 channel-count mismatch where Tier-2 played at half speed with sub-Nyquist distortion).
- **Wave 20 -- flush + engine_tick instrumentation, audio-off ablation tool, standalone diagnostic probes.** Five patches and one new build target for the Lever A diagnostic question (identifying what consumes the wave-18-measured 13.5 ms unbracketed flip-path cost). Patches: `patches/SDL/0035` (dual-path SDL flush instrumentation; slow-path `[uwfb-stat ...]` + fast-path `[direct-vesa-stat ...]` buckets, opt-in via `SDL_HINT_DOSKUTSU_FLIP_INSTR=1`), `patches/nxengine-evo/0087` (engine flip-phase decomposition; 7 phases per N=100 flips), `0088` (Lever B direct-VESA default-on with `=0` killswitch -- promotes the wave-18 +0.65 fps win), `0089` (`SDL_HINT_DOSKUTSU_AUDIO_OFF=1` deep-skip; bypasses `SDL_INIT_AUDIO` and `SoundManager::init()` for ablation), `0090` (engine_tick decomposition: `clear_screen` / `tsc_interp` / `mode_tick` / `debug_overlay` / residual; opt-in via `SDL_HINT_DOSKUTSU_FINE_INSTR=1` additive to FLIP_INSTR).
- **`make probes` target** + 8 standalone DJGPP diagnostic binaries under `tests/probes/` (gitignored): DACPROG (palette DAC programming cost), HWLOG (one-shot hardware introspection -- VBE info / Cirrus chip rev / PCI config space), DPMITHN (DPMI thunk overhead), L1FILL (cache + write-buffer characterization), PARTIAL (rect-flush crossover; postmortem of the wave-17.4 regression), YIELD (`SDL_Delay(0)` cost in SDL3-DOS scheduler), CFFSYNC (CF-card fsync wall-clock cost), IRQRATE (timer + keyboard IRQ rate via BIOS counter polling). Generic pattern rule for any future `tests/probes/*.c`. Bundled into iter tarballs for real-HW measurement runs.
- **Wave 19 -- Cirrus CL-GD5434 BLT engine probe** at `tests/cirrus-blt-smoke/probe.c` (lives on the `phase9/wave-19-cirrus-hwblit` branch -- not on main). Standalone DJGPP probe that programs the Cirrus BitBlt engine directly via I/O ports 0x3CE/0x3CF (GR0x20-0x33). Two test modes: scrolling-gradient correctness gate (proves chip programming model from the audit is correct; chip BLT engine reaches 19 MB/s effective for 76800-byte VRAM->VRAM copies) and dosmemput-delta measurement (compares back-surface staging at 0x80000 vs visible-FB at 0x00000 across 100 timed pairs).
...
- **KPI revised 30 -> 50 fps** (post wave-34, 2026-05-12). Wave-34 W34A1 cleared the prior 40 fps deadline target at 41.67 fps median (no SFX), confirming the 50 fps Cave Story design rate (`GAME_FPS=50` per `vendor/nxengine-evo/src/game.h:13`) is realistically reachable. Below 50 Hz, game logic runs at <design speed via 1:1 tick-to-render coupling in `gameloop()`; user-perceptual "feels laggy" persists even at 40 fps. Distance from current production (35.71 fps wave-37): ~14 fps / -8 ms inter_flip. Headline lever for the remaining gap = WaveBlaster MIDI offload via DSP-mediated path per SDL/0047 (NOT direct MPU-401 -- see Performance / wave-40 below).
- **README "Status" rewrite** to ~38 fps median on the reference PC (Intel Pentium OverDrive 83 MHz / Cirrus Logic CL-GD5434 / Creative SB16 PnP), with wave-37 binary as the production floor. README also de-branded "g2k" -> "reference PC" throughout, dropped links to gitignored internal docs, moved Status above Game Assets in the section order.
- **Audio default flipped** (wave 21 patch 0093): default config is now Tier-2 11025 Hz mono. Users who prefer the original Cave Story 22050 Hz stereo Organya sound can opt out with `SET SDL_HINT_DOSKUTSU_AUDIO_TIER2=0` before launching `DOSKUTSU.EXE`. The 11025 mono mix is rougher than the 22050 stereo original -- a deliberate trade for the +11 fps gain in music-heavy scenes (without it the title-attract scene drops to ~9 fps).
- **Lever B `SDL_HINT_DOSKUTSU_DIRECT_VESA` default flipped to ON** (wave 20 patch 0088). `=0` killswitch reverts to wave-17.6 SDL_RenderPresent path. Confirmed +0.65 fps in wave-18 measurement; promoted to default after instrumentation confirmed no regression risk.
- **README "Status" section** rewritten to reflect wave-20+21 closure. Replaces the prior session-wide-average "25.6 fps title" headline with scene-specific numbers (~20 fps in both music-heavy and lighter scenes; the 25.6 average was load-bearing-misleading because it weighted the title-attract pre-music loading blocks where `audio_mix=0`). Drops the wave-19-era "remaining ~12 ms of unidentified flush-path cost" framing -- that question is now closed.
...
- **Production fps trajectory waves 22-37 (Mimiga Village BK_PARALLAX heavy-music canonical scene):**
  - Wave-30 W30A1: **38.46 fps median** (best in project history at the time; +4.13 fps single-iter delta record via `nxengine-evo/0126` backdrop cache miss-streak direct-window; 6x reduction in 100+ ms stutter spike rate -- operator-perceptual stability win beyond fps median)
  - Wave-32: 40.00 fps measured but cross-iter replication (W33A1 38.46 fps same binary) showed ~2 fps scene-route variance noise floor; the wave-32 "win" was variance, not delta
  - Wave-34 W34A1: **41.67 fps median** (slot 0130 backdrop sub-region partial blit; first robust 40+ crossing). **Not shippable** because Pixtone SFX silent (gate-of-buffer-size-0 trap in `DOSSOUNDBLASTER_GetDeviceBuf`)
  - Wave-35 W35A1: **35.71 fps median** -- SFX correctness fix (SDL/0054 silence-detect) traded -5.96 fps. **This is the current production ship binary** (wave-37 sha `fe6bca0046d2`); correctness over peak fps per user-accepted decision
- **Waves 38-39-40 failed-arc** (lessons banked durably; full retrospective at `docs/internal/PHASE11-WAVE-38-39-RETROSPECTIVE.md`):
  - Wave-38: 3-lever stack default-ON (opaque-tile + BLT solid-fill + BULK_COPY async) + Mechanism A.2 fixed-timestep + STALL_REPORT instrumentation env-var. Measured **-14 fps from wave-37 baseline**. Per-lever attribution impossible because of the multi-lever ship; STALL_REPORT alone caused per-flip `getenv()` -> audio_thread starvation cascade. Reverted.
  - Wave-39: "recovery" iter with forced-rebuild discipline; Lever 2 finally engaged in binary; **-16 fps** (deepened wave-38). User-visible screen flashing + audio_thread starvation. Reverted.
  - Wave-40: WBMIDI direct MPU-401 probe returned `REFUTE_MPU_TIMEOUT` -- output-buffer-busy bit stuck after byte 1 on g2k DreamBlaster S2. Future WaveBlaster work must use DSP-mediated path per SDL/0047, NOT direct port writes.
- **Lessons -- single-lever-per-binary discipline now enforced.** ONE new mechanism per binary; probes ride along independently; instrumentation/banners/killswitches of the same mechanism don't count as additional mechanisms. Multi-lever PLAY-matrix killswitches don't decompose correlated regressions. Author-flagged caveats ("neutral or slight regression") ARE the data: ship default-OFF, killswitch-ON cell tests on real HW. Defensive rewrites require an identified failure stage; otherwise dispatch diagnostic-instrumentation pass first.
- **Wave 20 -- misc-slice question closed** via two converging data points (the wave-18-measured 13.5 ms unbracketed flip-path overhead is the SB16 audio IRQ-5 wall-clock interfering with the `SDL_GetTicksNS`-based flip_inner timer):
  - **W20A2 ablation** (`AUDIO_OFF=1` skips `SDL_INIT_AUDIO` so IRQ-5 is never hooked): `phase=total` median dropped from 18.05 ms (W20A audio-init-but-no-music early blocks) to **3.79 ms**. The bracketed-sum decomposition (drain + dirty + present + housekeeping ~ 3.71 ms) became exhaustive within DPMI thunk noise.
  - **W20C microbenchmarks**: `SDL_Delay(0)` = 0.515 us (per-flip yield_call eliminated as a contributor); CF fsync 5.3 ms each but only ~0.9 ms/flip distributed (tail latency, not median); non-audio IRQs at 18.2 Hz default contribute <0.3 ms/sec (eliminated). Component sum of remaining hypotheses ~ 13 ms ~ measured 13.5 ms gap.
- **Wave 20 -- engine_tick decomposition (W20B real-HW):** `mode_tick` 23.94 ms median = 91% of engine_tick budget; `clear_screen` 2.24 ms (bandwidth-bound; 3.5x slower than DOSBox-X); other phases negligible; residual 0.06 ms -- instrumentation exhaustive. `mode_tick` is the next optimization target for any future stretch beyond 30 fps; that's deep engine-level work (skip-when-unchanged lighting, sprite cull, AI state caching).
- **Wave 20 -- DOSBox-X-as-perf-proxy exception class identified.** CPU-bound integer/float arithmetic at calibrated cycles transfers ~1:1 (mode_tick 24.88 ms DOSBox-X -> 23.94 ms real HW = 1.04x scaling). Bandwidth/DMA/IRQ-timing paths still scale 30-50x (not faithful). Future engine-level optimization can iterate primarily in DOSBox-X for the CPU-bound bracket class without burning real-HW iter cycles.
- **Wave 20 -- Levers C/D/E disposition: all dropped.** Cross-validated zero counters ruled out palette DAC programming (`program_palette_calls=0/100` for 52 of 53 blocks; DACPROG probe FULL median 0.347 ms x 0 firings = 0 fps), cursor compositing (`cursor_composite_calls=0` all blocks; production direct-VESA path bypasses cursor entirely), and surface lifecycle churn (`housekeeping=0.04 ms` median; `gen_bumps=0/100`; `lazy_reinit_calls=0/100`). Saved Phase 9 from authoring three optimization patches against dead candidates.
- **Wave 21 -- Lever G real-HW results** (g2k Pentium OverDrive 83 MHz + Cirrus CL-GD5434 + UNIVBE 6.70 + Vibra16):
  - Tier-1 baseline (22050 stereo): heavy-music title attract ~9 fps (110 ms inter_flip), cmod first room ~19 fps (52 ms)
  - Tier-2 default (11025 mono, post-0094 audio-fix): heavy-music ~20 fps (50 ms), cmod ~20 fps (50 ms)
  - **Net: +11 fps in music-heavy scenes** (Tier-2 default vs Tier-1 opt-out via `=0`)
  - `phase=total` median: 46.4 ms -> 23.74 ms = **-22.6 ms/flip in heavy-music**, within the spec's predicted -3 to -22 ms range
  - `audio_mix` per flip: 27.1 ms -> 7.1 ms (~74% reduction)
- **Wave 18 -- Real-HW measurement on g2k:** wave-18 direct-VESA delivered **+0.65 fps** on the LFB path (`FORCE_LFB=1` + `DIRECT_VESA=1` -> 26.27 fps title) and **+0.27 fps** on the default banked path (`DIRECT_VESA=1` only -> 25.89 fps title). Both below the +2-5 fps prediction. The implementation is correct (100/100 successful presents per phase block, zero failures, snapshot generation stable) -- the savings are just small because the VRAM write itself dominates the original SDL `present` cost, not SDL dispatch glue.
- **Wave 19 path B real-HW result:** median back-surface dosmemput = 4.043 ms vs median visible-FB dosmemput = 4.042 ms (n=100 each, payload 76800 B). Delta = -0.001 ms (-0.0%). **`path-B-NO-OP`** verdict per the probe's decision logic. The display-contention hypothesis that wave-19 hung on is FALSE on Cirrus 5434 + UNIVBE 6.70 + this PCI bus. Chip BLT engine itself works fine (3.853 ms for 76800-byte VRAM->VRAM = 19 MB/s effective), but doesn't help -- wave-19 path B closed.
- **KPI change** (post-wave-19, sustained post-wave-21): project goal revised from 50 fps to **30 fps title** on the reference target. Wave 21 lands at ~20 fps default; **30 fps with original Organya music quality** requires Phase 10 hardware audio offload (OPL3 / WaveBlaster MIDI dual-backend) -- plan staged for a future implementation phase.
...
**First playable release.** Real-HW title fps **25.6 fps on the reference Pentium OverDrive 83 MHz / Cirrus CL-GD5434 / UNIVBE 6.70 / Vibra16 hardware** (g2k machine), measured on this binary's PLAY1 release-production config. Cumulative gain since Phase 8 baseline: **0.47 -> 25.6 fps = 54x improvement**. Cave Story is now perceptually playable on Pentium-class real DOS hardware.
...
**Cumulative since Phase 9 start:** real-HW title fps **0.47 -> 25.6** (54x improvement). Visual + audio + input all functional. Cumulative patch series: ~85 local patches across SDL3-DOS + NXEngine-evo. Per-flip budget at LFB-forced PLAY2 of wave 16.3: 16.4 ms drawcalls + 16.7 ms VRAM flush + 5.0 ms audio/engine glue.
...
- Real-HW result: **-7.1 fps regression** (25.7 -> 18.5). Per-rect dosmemput overhead (~3-5 ms x 2-4 rects/flush) dominated the bytes-saved win. Bytes-saved math correct; cost model missed per-call DPMI/banked setup overhead.
...
- Real-HW result: **-7.4 fps regression** (25.7 -> 18.3). Same magnitude as wave 17.4. Effective bandwidth: full-flush 4.4 MB/s, bbox-flush 1.07 MB/s. Per-byte cost is ~4x worse for partial vs full regardless of rect-vs-bbox structure -- unmodeled cost in SDL3-DOS partial dispatch path or our diff CPU.
...
- [X] H-A: display-start origin offset (refuted by audit)
- [X] H-B: display controller scan-cap at 64KB (refuted by wave-16.4 -- registers were already correct)
- [X] H-C: bank-switch broken (refuted by wave-16.1 sentinel match=1)
- [X] H-D: aperture-lies-past-64KB (refuted by wave-16.3 sentinel all-five-offsets match=1)
- [OK] Actual: legacy `blitPatternAcross` was simply not writing those pixels. Discovered when wave-17.3's bypass incidentally fixed it.
...
- Graphics options menu shows "640x480" (cosmetic UI bug; actual mode is 320x240 per modeset log)
...
- Analysis of GPLv3 implications of linking NXEngine-evo added to `PLAN.md sec. Licensing` and `THIRD-PARTY.md`
...
- First automated correctness gate (per `CLAUDE.md sec. Correctness Gate` item 1) -- passing
...
**The first end-to-end DJGPP + SDL3 build of NXEngine-evo.** `make nxengine` produces `build/doskutsu.exe` -- 5,866,918 bytes, COFF + go32 + CWSDPMI autoload, `stubedit minstack=2048k` confirmed. CLAUDE.md sec. Correctness Gate item 2 met (build-link gate); the title-screen runtime gate folds into Phase 6 (asset extraction now in flight).
...
**The framebuffer wall closes.** `make stage` + `tools/dosbox-launch.sh --stage` brings `DOSKUTSU.EXE` up under DOSBox-X to a visible 320x240 NXEngine title screen -- the first time the game has produced visible output in DOSBox-X. Six diagnostic + workaround patches in `patches/nxengine-evo/` plus one in `patches/SDL/` got us here. CLAUDE.md sec. Correctness Gate item 2's runtime gate is met; item 3 (playable content) and the Phase 7 stability gate (#23) defer to real-HW (g2k) testing per Phase 8.
...
- **`patches/nxengine-evo/0041-perf-cap-vesa-mode-bpp-at-16.patch`** -- sets `SDL_HINT_DOS_MAX_BPP="16"` at OVERRIDE priority before `SDL_Init`. Picks VESA mode 0x111 (640x480x16 bpp pitch=1280) instead of 0x112 (640x480x24 bpp pitch=2560) on Mach64. Per-frame VRAM bytes 921 KB -> 614 KB. **Measured 1.30x speedup on real HW.** Companion to the SDL hint definition below.
...
- `tests/phase8-runs/2026-04-26-16bpp-perf/` -- lever 0 measurement (~2.14 s/flip, 1.30x speedup).
