...
- **KPI change:** project goal revised from 50 fps to **30 fps title** on the reference target. Two consecutive perf hypotheses have closed under measurement (wave 18 SDL overhead = ~1.1 ms not ~5 ms; wave 19 display contention = 0 ms). 50 fps would require switching to original C-era NXEngine + DOS port rewrite (Lever 6, multi-month, uncertain outcome). The realistic ceiling on the current SDL3 + NXEngine-evo + DJGPP + PODP83 stack is ~30-35 fps, achievable by addressing the unidentified ~11.6 ms misc slice in the per-flip flush — diagnostic instrumentation needed first to identify what dominates.
