...
- **Wave 20 — misc-slice question closed** via two converging data points (the wave-18-measured 13.5 ms unbracketed flip-path overhead is the SB16 audio IRQ-5 wall-clock interfering with the `SDL_GetTicksNS`-based flip_inner timer):
  ...
  - **W20C microbenchmarks**: `SDL_Delay(0)` = 0.515 µs (per-flip yield_call eliminated as a contributor); CF fsync 5.3 ms each but only ~0.9 ms/flip distributed (tail latency, not median); non-audio IRQs at 18.2 Hz default contribute <0.3 ms/sec (eliminated). Component sum of remaining hypotheses ≈ 13 ms ≈ measured 13.5 ms gap.
- **Wave 20 — engine_tick decomposition (W20B real-HW):** `mode_tick` 23.94 ms median = 91% of engine_tick budget; `clear_screen` 2.24 ms (bandwidth-bound; 3.5× slower than DOSBox-X); other phases negligible; residual 0.06 ms — instrumentation exhaustive. `mode_tick` is the next optimization target for any future stretch beyond 30 fps; that's deep engine-level work (skip-when-unchanged lighting, sprite cull, AI state caching).
- **Wave 20 — DOSBox-X-as-perf-proxy exception class identified.** CPU-bound integer/float arithmetic at calibrated cycles transfers ~1:1 (mode_tick 24.88 ms DOSBox-X → 23.94 ms real HW = 1.04× scaling). Bandwidth/DMA/IRQ-timing paths still scale 30-50× (not faithful). Future engine-level optimization can iterate primarily in DOSBox-X for the CPU-bound bracket class without burning real-HW iter cycles.
- **Wave 20 — Levers C/D/E disposition: all dropped.** Cross-validated zero counters ruled out palette DAC programming (`program_palette_calls=0/100` for 52 of 53 blocks; DACPROG probe FULL median 0.347 ms × 0 firings = 0 fps), cursor compositing (`cursor_composite_calls=0` all blocks; production direct-VESA path bypasses cursor entirely), and surface lifecycle churn (`housekeeping=0.04 ms` median; `gen_bumps=0/100`; `lazy_reinit_calls=0/100`). Saved Phase 9 from authoring three optimization patches against dead candidates.
