...
#   T+~155s  : C:\<CELL>.DON sentinel written by shim (8.3-truncated name)
...
for cell in "${CELL_ARR[@]}"; do
    ...
    # Sentinel extension is `.DON` (3 chars), NOT `.DONE`: MS-DOS 8.3
    # truncates a 4-char extension, so `ECHO ... > C:\<CELL>.DONE` actually
    # creates C:\<CELL>.DON on the guest -- the poll/clear below must match
    # that 3-char name or the harness times out every cell (this was the
    # wave-51 "timeout without sentinel" cause). Fixed alongside the same
    # bug in tools/86box-probe-run.sh.
    {
        printf '@ECHO OFF\r\n'
        printf 'REM auto-generated by tools/86box-multi-cell-run.sh\r\n'
        printf 'CD C:\\DOSKUTSU\r\n'
        printf 'CALL %s.BAT\r\n' "$cell"
        printf 'ECHO CELL_DONE > C:\\%s.DON\r\n' "$cell"
    } > "$VMPATH/NEXT.BAT"
    mcopy -o -i "$SCRATCH_IMG@@$IMG_FAT_OFFSET" "$VMPATH/NEXT.BAT" "::/NEXT.BAT"

    # Clear any stale sentinel from a prior cell (.DON -- see NEXT.BAT note above)
    mdel -i "$SCRATCH_IMG@@$IMG_FAT_OFFSET" "::/${cell}.DON" 2>/dev/null || true
    ...
    while true; do
        ...
        if mcopy -i "$SCRATCH_IMG@@$IMG_FAT_OFFSET" "::/${cell}.DON" - 2>/dev/null | grep -q CELL_DONE; then
