...
> **⚠️ SUPERSEDED 2026-04-25 (corrected blockquote below).** The next blockquote was authored from a compaction-corrupted summary. Verifiable falsehoods: "zero `drawSurface` errors" (actual `build/stage/debug.log` shows 1647 errors over a full run, ~50/sec sustained on a shorter capture); "five pre-title-screen fatals fixed" overstates progress (one of the five — the `SDL_INVALID_PARAM_CHECKS=0` env var — is not a fix; the env var cannot suppress NULL-texture errors per the NULL guard at `vendor/SDL/src/SDL_utils_c.h:79`, see `PLAN.md § Plan Amendments § 2026-04-25 (revision)` for the structural reason). Preserved verbatim as a record of the compaction artifact.
...
> **Wall, accurate state 2026-04-25 (revision):** binary boots cleanly through full NXEngine init, reaches main loop + stage 72 (title screen), then the paint pipeline is blocked by sustained `Renderer::drawSurface: SDL_RenderTexture failed: Parameter 'texture' is invalid` errors (1647 over a full run; ~50/sec sustained on a shorter capture). DOSBox-X framebuffer stays black. Pre-title-screen fatals (CMake DJGPP data path / engine-data extraction / DOSBox-X LFN / INDEX8 palette) are fixed in commit `44fec06`; patches `0027`–`0030` landed since but none load-bearing for the wall. The texture-validation wall is **open and in active investigation**. See `PLAN.md § Plan Amendments § 2026-04-25 (revision)` for the full pickup state.

- [x] ~~All pre-title-screen fatals fixed: data-path baking (patch 0025), engine-data extraction (`wavetable.dat` + `stage.dat`), DOSBox-X LFN (`lfn = true`), INDEX8 palette (patch 0026), SDL3 texture-magic validation (`SDL_INVALID_PARAM_CHECKS=0`)~~ — **SUPERSEDED 2026-04-25:** the `SDL_INVALID_PARAM_CHECKS=0` entry is not a fix (env var arrives intact in the DOS process per `tests/sdl3-smoke/sdltest.exe`, but `SDL_object_validation = false` cannot bypass the NULL guard at `vendor/SDL/src/SDL_utils_c.h:79`); see corrected list below.
- [x] Pre-title-screen fatals fixed (revised list, commit `44fec06`): CMake DJGPP data-path baking (patch 0025), engine-data extraction (`wavetable.dat` + `stage.dat` via `scripts/extract-engine-data.py`), DOSBox-X LFN (`lfn = true` in `tools/dosbox-x*.conf` — DOSBox-X-only; real-HW Phase 8 follow-up tracked separately), INDEX8 palette in `zoom.cpp` (patch 0026)
- [ ] **Title screen actually renders to DOSBox-X framebuffer.** Paint pipeline blocked by sustained `SDL_RenderTexture` "Parameter 'texture' is invalid" errors. Patches `0027` (`SDL_SetRenderLogicalPresentation` letterbox) / `0028` (silent-site NULL-check + LOG_ERROR) / `0029` (`SDL_SetHint` programmatic for `INVALID_PARAM_CHECKS`) / `0030` (`drawSurface` NULL-check + early-return) all landed; none load-bearing for the wall yet. NULL-texture origin still under active investigation — see local task tracker for in-flight hypotheses; see `PLAN.md § Plan Amendments § 2026-04-25 (revision)` for the full state.
...
**Phase status:** Phases 0 / 1 / 2 / 5 / 6 / 7 closed. Phases 3' / 4'a / 4'b / 4'c / 4'd closed. **Phase 5 closed 2026-04-25 at `484efa7`** — first end-to-end DJGPP+SDL3 build. **Phase 6 closed 2026-04-25 at `44fec06`** — `data/` populated with Cave Story freeware + NXEngine engine support data. **Phase 7 framebuffer wall closed 2026-04-25 at `7372374`** — title screen + first lab room render pixel-perfect under DOSBox-X via the diagnostic ladder of patches `0027`–`0032` + `SDL/0002`; load-bearing fixes are `0030` (drawSurface NULL early-return) and `0032` (`SDL_HINT_DOS_ALLOW_DIRECT_FRAMEBUFFER` flip). Phase 7 stability gate (#23) deferred — real-HW (g2k) testing is now the primary validation path. **Phase 8 prerequisite work in flight 2026-04-25**: `0033+` 8.3 source-rename to make the binary boot on real DOS 6.22 without LFN (per `docs/PHASE8-LFN-RENAME-PLAN.md`); `tests/dpmi-lfn-smoke/probe.c` to verify whether LFN-TSR-via-CWSDPMI works for future mod-loading needs; `make dist` target ready for the dist zip once Phase 8 closes. See `CHANGELOG.md` for the full Phase 7 closure entry.
...
- **Latent SDL3-DOS bug — `DOSVESA_InvalidateCachedFramebuffer` init-state-leak.** Surfaced 2026-04-25 by patch `SDL/0002`'s diagnostic. Patch `0032` works around it by flipping `SDL_HINT_DOS_ALLOW_DIRECT_FRAMEBUFFER=1` early in `main`, which causes the fast-path *setup* to populate `fb_state` even though the runtime fast-path guard at `vendor/SDL/src/video/dos/SDL_dosframebuffer.c:494` still fails (`InvalidateCachedFramebuffer` clears `direct_fb` but leaves the rest, so the normal-path banked-mode blit at line 806 inherits a usable `fb_state` by side-effect). Architecturally messy but not blocking — `0032` keeps things working. **Hold for real-HW evidence** before authoring `patches/SDL/0003-` to fix the underlying mechanism: if real HW behaves like DOSBox-X, no fix needed; if it diverges, the divergence guides what the fix should look like. No-action by upstream-patches policy (see memory note `sdl_ai_policy.md`); fix lands locally if needed.
