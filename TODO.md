...
**Phase status:** Phases 0 / 1 / 2 / 5 / 6 closed. Phases 3' / 4'a / 4'b / 4'c / 4'd closed (per task tracker #28, #30, #31, #32, #33). **Phase 5 closed 2026-04-25 at `484efa7`** — first end-to-end DJGPP+SDL3 build. **Phase 6 closed 2026-04-25 at `44fec06`** — `data/` populated with Cave Story freeware + NXEngine engine support data, `wavetable.dat` + `stage.dat` extractor authored, `data/base/` subdir convention superseded. **Phase 7 partially open** — pre-title-screen fatals fixed (CMake DJGPP data path / engine-data extraction / DOSBox-X LFN / INDEX8 palette; commit `44fec06`); the title-screen-doesn't-render-to-framebuffer wall (sustained `SDL_RenderTexture` "Parameter 'texture' is invalid" errors blocking the paint pipeline) is **open and in active investigation** — patches `0027`–`0030` landed since `44fec06`, none load-bearing for the wall yet. See `PLAN.md § Plan Amendments § 2026-04-25 (revision)` for the pickup state.
