 ...
 * + file the game's OPL3 backend loads). g_bank_count == 0 means the file was
 ...
 * count*12 patch bytes). Ported from the game's OPL3 bank loader. Returns 1 on
 * success; on ANY failure leaves g_bank_count = 0 so the 8-patch placeholder is
 * used (an incomplete install still previews). fopen "rb" -- DJGPP text mode
 * would CRLF-corrupt the binary bank. */
...
int cs_opl3midi_bank_programs(void) { return g_bank_count; }

/* GM program (0-127) -> patch. A loaded opl3bank.dat program wins; otherwise the
 * 8-patch family bucket (MidiBackendOpl3.cpp). */
