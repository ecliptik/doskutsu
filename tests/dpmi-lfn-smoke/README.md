# tests/dpmi-lfn-smoke — Phase 8 DPMI LFN propagation probe

Smoke test answering the gating empirical question for Phase 8's LFN strategy
(see `docs/PHASE8-LFN-DECISION.md`): **does CWSDPMI's INT 21h reflector pass
LFN-family calls (function codes 7140h-71A8h) through to a real-mode TSR
loaded under plain MS-DOS 6.22?**

If yes → bundle LFNDOS.COM (or DOSLFN.COM as fallback) per the memo's
option A. If no → source-rename every long-named asset per option B (the
multi-week fallback).
...
| Test | Call | Expected on DOSBox-X `lfn=true` | Expected on g2k + LFN TSR | Expected on g2k + no LFN TSR |
|---|---|---|---|---|
...
## How to run on g2k real hardware (the actual question)

Per `docs/PHASE8-G2K-CHECKLIST.md`'s deployment shape:

1. Copy `build/dpmi-lfn-smoke/probe.exe`, `vendor/cwsdpmi/cwsdpmi.exe`,
   `WAVETABL.DAT`, and `wavetable.dat` to a CF card.
2. Boot g2k normally (no LFN TSR loaded — control run).
...
- LFNDOS-loaded run shows `PROBE: long_int21_716c PASS handle=0x????` →
  **option A confirmed. Ship LFNDOS.** Update `Makefile` `dist` target,
  `THIRD-PARTY.md`, `docs/BOOT.md` per the memo's "What to do at Phase 8
  time" §3.
- LFNDOS-loaded run still shows `PROBE: long_int21_716c FAIL doserr=0x02`
  ...
  → **option B mandatory. Source-rename**, per `PLAN.md` task #27 and the
  memo's "What to do at Phase 8 time" §4.

## Why the probe doesn't ship LFNDOS itself

Per user direction (see memory note `sdl_ai_policy.md`), we do NOT vendor
external project sources or binaries into this repo without an established
permission and a track record (CWSDPMI is the one exception, established
pre-policy). The probe's job is to ANSWER the LFN-via-DPMI question; the
*deployment* of LFNDOS to g2k is a Phase 8 implementation step taken only
after the probe confirms it would actually help. Until then, the probe
runs on a g2k machine that has LFNDOS independently installed by the
operator (per `gitlab.com/FreeDOS/drivers/lfndos`'s release tarball).
...
- DPMI LFN-translation caveat: `github.com/adoxa/doslfn` `doslfn.asm`
  header block (cited in `docs/PHASE8-LFN-DECISION.md` § DPMI compatibility).
