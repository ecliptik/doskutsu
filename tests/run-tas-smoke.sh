...
#   1. RECORD mode -- DOS_PORT_TAS_RECORD=PLAY.TAS, AUTO_EXIT_TICK=100;
#      captures ~2 seconds of title-screen idle input into PLAY.TAS,
#      then auto-exits cleanly via game.running = false.
#   2. REPLAY mode -- DOS_PORT_TAS_REPLAY=PLAY.TAS, AUTO_EXIT_TICK=100;
...
# DOS_PORT_LOG_TAG=RECORD makes the engine log to /DOSKUTSU/RECORD.LOG
# (per patch 0069), so we can keep RECORD-pass and REPLAY-pass logs
# disjoint.
RECORD_BAT="$STAGE_DIR/TASREC.BAT"
cat > "$RECORD_BAT" <<EOF
@echo off
SET DOS_PORT_LOG_TAG=RECORD
SET DOS_PORT_TAS_RECORD=PLAY.TAS
SET DOS_PORT_TAS_AUTO_EXIT_TICK=$EXIT_TICK
