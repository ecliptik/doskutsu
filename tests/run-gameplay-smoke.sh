...
BANNER_LABEL=(
  ...
  "BG-tile census instrumentation (patch 0327; required -- one banner per process emitted unconditionally in Renderer::initVideo, narrating ENABLED (opt-in) via SDL_HINT_DOSKUTSU_MDS_TBG_INSTR=1 vs DISABLED (default) when unset; fires deterministically at video init every boot so present in smoke; default smoke leaves the hint unset so the DISABLED variant is expected; the ENABLED path emits per-100-tick [mds-tbg-stat]/[mds-tbg-tiles] BG-tile census + occlusion-headroom lines)"
