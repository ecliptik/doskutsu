...
BANNER_REGEX=(
  ...
  "Renderer::initVideo: BG-tile census instrumentation (ENABLED \(opt-in\)|DISABLED \(default\))"
  ...
  "audio: SDL/0070 v2 SfxSynth probe pix_active histogram \\+ irq_delta fold-in"
  "audio: SDL/0071 sfx_synth IRQ-mix: (ENABLED|DISABLED|REQUESTED)"
  ...
  "audio: SDL/0071 sfx_synth IRQ-mix: ENABLED"
  ...
  "audio: SDL/0108 sfx_synth IRQ-mix format: (S16-stereo|U8-stereo|U8-mono) \(device is_16bit=[01] channels=[12]\)"
  ...
  "tile-bbox-skip: (ENGAGED \\(default since 0310\\)|DISABLED \\(killswitch =0\\)) -- off-screen tile slots"
  "surface-extent: box=[0-9]+x240@"
  ...
  "Renderer::initVideo: tile dword-copy lever (ENABLED \\(opt-in\\)|DISABLED \\(default\\)) -- whole-tile"  "pin-native-mode: (ENGAGED \\(default\\)|STOOD DOWN \\(DOS_PORT_PIN_NATIVE_MODE=0\\)) --"
  "\\[shot-dump\\] ARMED n=[0-9]+ first=[0-9]+ last=[0-9]+ dir='[^']*' \\(DOS_PORT_SHOT_TICKS="
...
BANNER_LABEL=(
  ...
  "wave-41 TAS record/replay engaged (patch 0135; optional -- emits only when DOS_PORT_TAS_RECORD or _REPLAY env var is set; default smoke runs with neither so banner absent is correct)"
  ...
  "wave-42 #19 GM_VARIANT engaged (patch 0137; optional -- emits when SDL_HINT_DOS_AUDIO_MIDI_GM_VARIANT=v1 or v2 selects data/orgmid1// or data/orgmid2/; default smoke leaves env unset so default-orgmid path runs and banner is absent)"
  ...
  "wave-46 default OPL3 backend (patch 0139; optional -- emits when SDL_HINT_DOS_AUDIO_BACKEND is unset, which is the wave-46 production default; absent under explicit AUDIO_BACKEND=organya/wb/auto override)"
  ...
  "wave-48 BG skip-when-unchanged decision (patch 0143; default flipped to OFF by patch 0157/wave-61 after it shipped the backdrop-black flicker; patch 0313 FIXED that flicker (the upstream clearScreen in tick_render_submit is now gated on the same latched predicate DrawScene reads) and the lever stays default-OFF pending hardware re-measurement; optional -- one banner per process emitted on first call to map_backdrop_should_skip_clear_and_draw, narrating ENABLED (opt-in via SDL_HINT_DOS_BG_SKIP_WHEN_UNCHANGED=1) vs DISABLED (default); fires deterministically on first DrawScene call so should emit even under DOSBox-X smoke if engine reaches DrawScene at all)"
  ...
  "wave-50 VRAM-resident page-flip chip-detect (patch SDL/0061; optional -- one banner per process emitted on first call to _vram_resident_check_enabled inside SDL_DOSVesaDirectPresentFull's divert OR via SDL_DOSVesaPagePresent, narrating ENABLED (Cirrus 5434 + sanity-check pass) vs DISABLED (killswitch off / chip not 5434 / sanity fail); DOSBox-X smoke typically emits DISABLED (emulated chip != Cirrus 5434) UNLESS SDL_HINT_DOS_VRAM_RESIDENT=1 is set; real-HW g2k cycle-1 iter sets the env var so ENABLED is expected; absence on real-HW with env set indicates the killswitch was unset OR the present path never engaged the divert -- check SDL_HINT_DOS_CIRRUS_BLT not =0 either since the chip-detect is shared with SDL/0060)"
  ...
  "wave-51 asm opaque-span blit lever (patch 0145; optional -- one banner per process emitted in Renderer::initVideo, narrating ENABLED via SDL_HINT_DOS_ASM_BLIT=1 vs DISABLED (default-OFF for wave-51); fires deterministically at video init so emits under DOSBox-X smoke; default smoke leaves the hint unset so DISABLED is the expected smoke verdict)"
  "wave-51 blit-decomp instrumentation (patch 0146; optional -- one banner per process emitted in Renderer::initVideo, narrating ENABLED via SDL_HINT_DOS_BLIT_DECOMP=1 vs DISABLED (default-OFF); the .* in the regex spans the ENABLED variant's extra '_blit_indexed sub-decomp' words; default smoke leaves the hint unset so DISABLED is expected; the two INSTR PLAY cells in the wave-51 matrix exercise the ENABLED path)"
  "wave-52 mds-scroll-decomp instrumentation (patch 0147; optional -- one banner per process emitted in Renderer::initVideo, narrating ENABLED via SDL_HINT_DOS_MDS_DECOMP=1 vs DISABLED (default-OFF); fires deterministically at video init; default smoke leaves the hint unset so DISABLED is expected; the wave-52 PLAY1 cell exercises the ENABLED path -- per-100-scene-draws [mds-decomp-stat]/[mds-decomp-route]/[mds-decomp-pass]/[mds-decomp-frames] emit lines)"
  "BG-tile census instrumentation (patch 0327; required -- one banner per process emitted unconditionally in Renderer::initVideo, narrating ENABLED (opt-in) via SDL_HINT_DOS_MDS_TBG_INSTR=1 vs DISABLED (default) when unset; fires deterministically at video init every boot so present in smoke; default smoke leaves the hint unset so the DISABLED variant is expected; the ENABLED path emits per-100-tick [mds-tbg-stat]/[mds-tbg-tiles] BG-tile census + occlusion-headroom lines)"
  "wave-53 skip-redundant-clear lever (patch 0149; optional -- LOG_INFO banner narrating ENABLED via SDL_HINT_DOS_SKIP_REDUNDANT_CLEAR=1 vs DISABLED (default-OFF); INFO-level so it only appears when the run logs at INFO -- tagged runs, or untagged with DOS_PORT_LOG_VERBOSE=1; absent on a plain untagged WARN-level run, which is expected, not a failure)"
  "wave-53 Logger log-level banner (patch 0148; optional -- emitted directly by Logger::init regardless of level, narrating INFO vs WARN; untagged production runs default to WARN, tagged runs and DOS_PORT_LOG_VERBOSE=1 force INFO; the runtime witness that the wave-53 WARN-when-untagged default engaged)"
  "wave-54 PERF_MODE level banner (patch 0150; optional -- LOG_INFO emitted on the first perf_mode_level() call, which is the first map_draw_backdrop call, narrating level=0 faithful (default) / 1 smooth / 2 fast; INFO-level so it appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1, absent on a plain untagged WARN-level run which is expected not a failure; the runtime witness that the SDL_HINT_DOS_PERF_MODE infra was reached)"
  "wave-55 cut B2 decorative-FG-detail first-fire (patch 0151; optional -- LOG_INFO emitted once on the first frame B2 actually filters a purely-decorative foreground tile (attribute word == TA_FOREGROUND exactly); the two-witness runtime side complementing strings|grep \"perf-mode B2\" which proves embed only; fires ONLY when the smoke run boots with SDL_HINT_DOS_PERF_MODE>=1 AND reaches a map that has at least one decorative FG tile -- a level-0 boot, or a scene whose FG layer is all collision/slope tiles, never fires it, which is expected not a failure. NOTE the wave-54 A2 banner was removed: A2 gain-collapsed on real HW and its hunk was reverted by patch 0151)"
  "wave-57 FIXED_TIMESTEP killswitch decision (patch 0153; default flipped ON by patch 0159 wave-63, the 1.0 release; optional -- LOG_INFO emitted on the first fixed_timestep_active() call (the first gameloop iteration), narrating ENABLED (default; logic 50 Hz decoupled from render) vs DISABLED (SDL_HINT_DOS_FIXED_TIMESTEP=0 killswitch; 1:1 logic/render); INFO-level so it appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1, absent on a plain untagged WARN-level run which is expected not a failure; the runtime witness that the Track B B2 accumulator engaged -- on a default boot the ENABLED variant confirms the 1.0 default 50 Hz fixed-timestep mode)"
   "SDL/0063 WaitDevice ring-drain timeout (486-campaign Bug 2 quit-to-DOS hang fix; optional -- emitted at audio OpenDevice (early boot) on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG -- the smoke gate greps both), default-ON; the disabled variant emits only when SDL_HINT_DOS_DOS_AUDIO_WAITDEVICE_TIMEOUT=0; the runtime witness that the SDL/0063 ring-drain teardown-deadlock backstop is live -- the quit-hang itself reproduces only on real 486DX2 hardware, so this banner is the build-qa runtime-invocation confirmation, not a hang test)"
   "SDL/0066 audio-thread hard-park (486-campaign Bug 2 quit-to-DOS hang fix proper -- the targeted fix that SDL/0063 did not deliver; optional -- emitted at audio OpenDevice (early boot) on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG), default-ON; the disabled variant emits only when SDL_HINT_DOS_DOS_AUDIO_SHUTDOWN_PARK=0; the runtime witness that the SDL/0066 audio-thread teardown-park is live -- the engagement banner audio: SDL/0066 hard-park ENGAGED is emitted only at quit when SDL_DOSAudioBeginShutdown fires, so DOSBox-X smoke confirms the active-at-init banner but not the engagement; the actual hang reproduces only on real 486DX2 hardware, so this active-at-init banner is the build-qa runtime-invocation confirmation, not a hang test)"
   "SDL/0067 SDL_DOSAudioPump export available (audio mid-gap-service export for 486-campaign Bug 1 / gap-driven SFX stutter; optional -- emitted at audio OpenDevice (early boot) on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG); unconditional -- the SDL-side export is always available; what gates the engine call-sites is the engine-side SDL_HINT_DOS_AUDIO_MIDGAP_PUMP env var read by nxengine-evo/0162 (Leg A pump-call patch). The active-at-init banner is the runtime witness that the SDL_DOSAudioPump symbol is wired and the audio backend's OpenDevice ran; nx-engine emits its own engine-side engagement banner by default (default-ON; the killswitch SDL_HINT_DOS_AUDIO_MIDGAP_PUMP=0 emits the DISABLED variant instead). The actual SFX-stutter mitigation reproduces only on real DX2-66 hardware (the cooperative-scheduler gap-driven underrun); DOSBox-X smoke confirms the export embed + boot path, not the perf mitigation)"
   "SDL/0068 SDL_DOSAudioPump TryLock+timeout backstop (cooperative-scheduler-starvation guard for the SDL_DOSAudioPump path; optional -- emitted at audio OpenDevice (early boot) on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG), default-ON with 100 ms wall-clock cap; the disabled variant emits only when SDL_HINT_DOS_DOS_AUDIO_PUMP_TIMEOUT=0; the runtime witness that the SDL/0068 backstop is live -- the wedge the backstop guards against (build-qa #84 bisect, wave-84 combined-iter: SDL_DOSAudioPump's implicit-blocking-lock starves under DOSBox-X SB16 emulation when the playback thread's WaitDevice never DOS_Yields) reproduces only under that DOSBox-X cadence regime, so on emulation the backstop's presence is what makes the gate-pass possible; on real DX2-66 the natural ~43 Hz SB16 IRQ-5 makes WaitDevice yield per cycle and the backstop is unused defensive insurance. SDL/0068 active means SDL_DOSAudioPump can be called from main without the wave-84 wedge regardless of which side the regime falls)"
   "SDL/0069 cumulative silent-IRQ counter export (Pixtone multi-source probe hypothesis-C signal; optional -- emitted at audio OpenDevice (early boot) on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG), unconditional -- no killswitch, no env var; the SDL-side counter dos_port_audio_silent_irq_count is always live as a single 32-bit volatile increment in the IRQ silent-branch alongside the existing isr_consecutive_silent_irqs (which resets on any non-silent IRQ AND on DSP-resume service, losing intra-block peaks). The sibling counter is monotonic; the engine-side Pixtone probe reads it via extern at audit-flush boundary and computes block-over-block deltas to detect ring-empty bursts during rapid-fire SFX moments. The active-at-init banner is the runtime witness that the counter symbol is wired and the audio backend's OpenDevice ran; nx-engine's Pixtone probe (engine-side, opt-in via SDL_HINT_DOS_SFX_SYNTH_PROBE=1) emits its own engagement banner separately. See docs/internal/PIXTONE-MULTISOURCE-DESIGN.md sec.2.1 #2 + sec.4 #4)"
   "SDL/0070 v2 Pixtone probe pix_active histogram + irq_delta fold-in (resolves v1 hypothesis-E instrumentation gap + sec.21.4.1 silent_irq numeric anomaly; optional -- emitted at audio OpenDevice (early boot) on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG). The active-at-init banner is the runtime witness that the v2 probe machinery is wired. The pix_active histogram + irq_delta fields extend the existing [sdl-audiocb] block emit; their engagement is gated on FINE_INSTR=1 (rides on g_c_sfx_audiocb_instr_active from SDL/0064; no new env var). When FINE_INSTR=1, every 100 PlayDevice invocations the [sdl-audiocb] block emits with pix_active min/med/p90/max samples=N + irq_delta=M. v1's engine-side histogram (nxengine 0165's SampleSfxSynthActiveBucket hooked to organyaStreamCallback) fired only under the Organya backend; OPL3 (wave-46 default since 2026-05-14) bypassed it -> samples=0 in v1 iter -> hypothesis-E verdict. v2's SDL-side sampler at the top of DOSSOUNDBLASTER_PlayDevice is the unified-cadence point that fires regardless of music backend (OPL3 / Organya / WaveBlaster all funnel through SDL3_mixer -> PlayDevice). The irq_delta fold-in (snapshot of dos_port_audio_irq_count between block emits) is the IRQ-rate corroborator that resolves the sec.21.4.1 anomaly (v1 silent_irq ~750/block vs design-assumed 43 Hz IRQ predicting ~190 IRQs/block over 4.4 s). See docs/internal/PIXTONE-PROBE-V2-DESIGN.md sec.2 + sec.5)"
   "SDL/0071 pixtone IRQ-mix killswitch decision (Lever 3 structural Bug-2 fix; optional -- LOG_INFO banner emitted at audio OpenDevice on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG), one of three variants: ENABLED (DEFAULT-ON since v1.0.1, patch SDL/0075; set SDL_HINT_DOS_SFX_SYNTH_IRQ_MIX=0 to disable), DISABLED (killswitch =0), or REQUESTED (default-on but backend-gate refused because SDL_HINT_DOS_AUDIO_BACKEND=organya forces L3 off per LEVER3 sec.5.4). Lever 3 moves Pixtone PCM mix from the cooperative-thread SDL3_mixer ring path INTO the SoundBlasterIRQHandler IRQ-5 ISR -- the IRQ that owns the DMA buffer also does the SFX mix, bypassing the cooperative scheduler entirely for the SFX path. Structural answer to wave-21 / sec.21 / #120 hypothesis-C verdict (cooperative-scheduler audio-thread starvation; up to 1701 ms max single-callback latency on real DX2-66). Default-OFF first iter per [[author_flagged_caveat_overrides_let_data_decide]] + [[single_lever_per_binary_or_else_attribution_impossible]] discipline; SDL3_mixer fallback path stays alive when off so a real-HW DPMI-fault inside the ISR mitigates to disabled rather than freezing. The active-at-init banner is the runtime witness that the Lever 3 wiring engaged or was correctly refused. The actual stutter-elimination effect reproduces only on real DX2-66 hardware; DOSBox-X smoke confirms the banner / boot path but not the cooperative-scheduler bypass behavior per [[dosbox_not_proxy]]. Engine-side routing of Pixtone::play through SDL_DOSSfxSynthStart is in nxengine-evo/0167. See docs/internal/LEVER3-PIXTONE-IRQ-MIX-DESIGN.md sec.1-7 + sec.10 contract lock + 486-CLASS-CAMPAIGN-FINDINGS.md sec.21 + #120 sec.22)"
   "SDL/0072 midi tick from ISR killswitch decision (Lever 2b structural Bug-4 fix; optional -- LOG_INFO banner emitted at audio OpenDevice on the SDL-log channel (<TAG>SDL.LOG / SDLDBG.LOG, NOT the engine DEBUG.LOG), one of two variants: ENABLED (DEFAULT-ON since v1.0.1, patch SDL/0075; set SDL_HINT_DOS_MIDI_ISR_TICK=0 to disable) or DISABLED (killswitch =0). Lever 2b moves MidiScheduler::tick from the cooperative main-loop path INTO the SoundBlasterIRQHandler IRQ-5 ISR -- the IRQ fires at ~43 Hz Tier-2 (about 2x rendered fps on heavy-music DX2-66 scenes), decoupling MIDI tempo accuracy from render-rate. Structural answer to 486-campaign Bug 4 (music-tempo wobble under sub-25 fps gameplay). No backend-gate -- Lever 2b is orthogonal to the music backend choice (the engine simply doesn't register a callback when running under Organya, which doesn't use MidiScheduler; under OPL3 / WB / midi-stub, the engine registers an extern \"C\" shim that invokes MidiScheduler::tick_isr(now_ms) per LEVER2B sec.1.2). The engine registers an ISR-safe callback via SDL_DOSMidiTickRegister (declared in vendor/SDL/include/SDL3/SDL_dosmidi_tick.h); the ISR computes monotonic cumulative now_ms via inter-IRQ delta accumulation through the existing SDL_DOSAudioReadTimer + SDL_DOSAudioTimerDeltaUs primitives. Default-OFF first iter per [[author_flagged_caveat_overrides_let_data_decide]] + [[single_lever_per_binary_or_else_attribution_impossible]] discipline; main-loop MidiScheduler::tick fallback path stays alive when off so a real-HW DPMI-fault inside the registered callback mitigates to disabled rather than freezing. The active-at-init banner is the runtime witness that the Lever 2b wiring engaged. The actual tempo-perception effect reproduces only on real DX2-66 hardware; DOSBox-X smoke confirms the banner / boot path but not the render-rate decoupling behavior per [[dosbox_not_proxy]]. Engine-side routing + tick_isr() body + game-pause-coherent stop/start at pause transitions are in nxengine-evo/0168. See docs/internal/LEVER2B-MIDI-ISR-TICK-DESIGN.md sec.1-7 + sec.8 contract lock + 486-CLASS-CAMPAIGN-FINDINGS.md sec.17 + sec.20)"
  "0162-v2 Leg A audio mid-gap pump (486-campaign Bug 1 stage-load ~1-second-freeze fix, v2 -- the v1 iter showed PLAY1 missed Shack from accumulator-desync, v2 adds FIXED_TIMESTEP freeze/thaw around load_stage and flips default to OFF per flush-instr's safer-landing recommendation; optional -- LOG_INFO emitted on the first MIDGAP_PUMP call (first stage entry), narrating ENABLED (opt-in via SDL_HINT_DOS_AUDIO_MIDGAP_PUMP=1) vs DISABLED (default; the lever is off until v3 promotes it back to default-ON after v2 iter validation); INFO-level so it appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1, absent on a plain untagged WARN-level run which is expected not a failure; this is the engine-side banner -- complements the SDL-side 'audio: SDL/0067 SDL_DOSAudioPump export available' banner above (the SDL banner confirms the export is wired and reachable; this banner confirms nxengine-evo/0162-v2's killswitch parser engaged and the engine call-sites are live). On a default boot the DISABLED variant is expected (lever off); cells that SET the hint to 1 emit ENABLED and exercise the 6 inter-loader pumps in load_stage + the freeze/thaw RAII guard. The v2 pumps use 'while (SDL_DOSAudioPump()) { }' to top up the ring to full per seam, and the FtAccumulatorGuard at load_stage entry snapshots accum_ns + restores it at every return path so wall-clock spent in load_stage is invisible to the catch-up tick_state count (closes the v1 TAS desync). See docs/internal/AUDIO-MID-GAP-SERVICE-DESIGN.md + 486-CLASS-CAMPAIGN-FINDINGS.md sec.17)"
  "0163 engine data cache (486-campaign Bug 1 stage-load freeze fix -- the structural replacement for Leg A; optional -- LOG_INFO emitted on the first nxe::cache::enabled() call (engine init, right after SDL_Init), narrating ENABLED (opt-in via SDL_HINT_DOS_DATA_CACHE=1) vs DISABLED (default; the v1 lever is off until iter validation); INFO-level so it appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1, absent on a plain untagged WARN-level run which is expected not a failure; the runtime witness that the cache module is wired and the killswitch parser engaged. On the ENABLED path, additional per-dir progress lines emit -- 'data cache: preloading Stage/', 'data cache: preloading Npc/', 'data cache: preloading top-level *.pbm', 'data cache: preloading pxt/', and the final 'data cache: preload complete -- N files / M bytes resident / S skipped (X bytes malloc-failed)' -- which name the access-pattern preload order and the partial-cache resilience. The cache replaces audio mid-gap pumping (0162-v2) as the structural fix: a single uninterruptible .pbm fread on real DX2-66 CF exceeds the audio ring depth, so caching the .pbm bytes in RAM is the only way to make stage transitions fast enough for the audio ring not to underrun. See docs/internal/DATA-CACHE-DESIGN.md + 486-CLASS-CAMPAIGN-FINDINGS.md sec.18)"
  "0164 Leg B audio tick-boundary pump (486-campaign Bug 2 continuous-SFX-stutter fix; optional -- LOG_INFO emitted on the first tickpump_active() call (the first run_tick_fixed iteration), narrating ENABLED (opt-in via SDL_HINT_DOS_AUDIO_TICKPUMP=1) vs DISABLED (default; the v1 lever is off until iter validation); INFO-level so it appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1, absent on a plain untagged WARN-level run which is expected not a failure; the runtime witness that the tick-boundary TICKPUMP macro is wired and the killswitch parser engaged. On the ENABLED path, a single TICKPUMP() call site fires in run_tick_fixed immediately before game.tick_render_submit() once per displayed frame (both 1:1 and catch-up paths converge here), running while(SDL_DOSAudioPump()){} to top up the SB16 ring to full -- bounded by ring depth (~16 iterations worst-case from empty, 1-2 in steady-state at 30 fps); in the silent case (no SFX/Organya/MIDI) the loop body executes zero times so the cost is just the cached int compare. Complements 0162-v2 Leg A (stage-load freads) by covering the steady-state gameplay path where heavy render frames defer the cooperative-scheduler audio thread past the ring depth (wave-38 cooperative-scheduler starvation pattern). Wave-36 trap clear: pump invokes SDL_DOSAudioPump only -- no game.tick_state, no drawcall submit, no catch-up re-entry. On a default boot the DISABLED variant is expected (lever off); the PLAY1 cell of the audio-pipeline iter SETs the hint to 1 to engage. See docs/internal/AUDIO-PIPELINE-CAMPAIGN-DESIGN.md sec.1.2)"
  "0165 Pixtone multi-source probe (486-campaign Bug 2 continuous-SFX-stutter probe; optional -- LOG_INFO emitted on the first pixtone_probe_active() call (the first audit-flush boundary at flip 100), narrating ENABLED (opt-in via SDL_HINT_DOS_SFX_SYNTH_PROBE=1) vs DISABLED (default; the lever is off until the operator engages it for the probe iter); INFO-level so it appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1, absent on a plain untagged WARN-level run which is expected not a failure; the runtime witness that the probe killswitch parser engaged and the [pixtone-multi] block emit is wired. On the ENABLED path, one [pixtone-multi block=N samples=N active min=X med=X p90=X max=X live=N stoptrack=N polarstar=N silent_irq=N] line per 100-flip block from _maybe_flush_pixtone_multi_audit -- the active-count histogram (6 buckets [0]/[1]/[2]/[3-5]/[6-10]/[11+]; sampled at audio-buffer-fill ~43 Hz Tier-2 via NXE::Diag::SampleSfxSynthActiveBucket called from organyaStreamCallback) + block-over-block deltas for g_pixtone_stoptrack_count (MIX_StopTrack-on-preempt-and-replay events, channel-pressure indicator) + g_pixtone_polar_star_count (combined SND_POLAR_STAR_L1_2 + L3 fires, SFX-storm-rate indicator) + dos_port_audio_silent_irq_count (SDL/0069 monotonic; scheduler-starvation indicator). Discriminates 5 hypotheses A-E (per-callback mix CPU / track-state transition / scheduler starvation / IRQ jitter / re-diagnose) for the contingent v2 fix author. Counter increments are unconditional; emit + histogram bucket-inc gated on probe_active() -- cache-OFF byte-identical-at-rendered-frame to pre-0165. On a default boot the DISABLED variant is expected (lever off); the PLAY1 cell of the probe iter SETs the hint to 1 to engage. See docs/internal/PIXTONE-MULTISOURCE-DESIGN.md)"
  "0167 Lever 3 engine-side routing (Pixtone IRQ-mix; engine companion to patches/SDL/0071; optional -- LOG_INFO banner emitted on the first pixtone_irq_mix_active() call (the first Pixtone::play / Pixtone::stop / setVolume hit after engine init), narrating ENABLED (DEFAULT-ON since v1.0.1, patches SDL/0075 + nxengine-evo/0170; set SDL_HINT_DOS_SFX_SYNTH_IRQ_MIX=0 to disable) vs DISABLED (killswitch =0, or backend-gate refused under Organya). INFO-level; appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1; absent on a plain untagged WARN-level run which is expected not a failure. The runtime witness that the engine-side routing branches engaged. On the ENABLED path, Pixtone::play routes non-resampled SFX through SDL_DOSSfxSynthStart (SDL/0071) instead of MIX_PlayTrack, bypassing the cooperative-scheduler audio thread for the SFX path (hypothesis-C mitigation per #120 sec.22). Pixtone::stop routes to SDL_DOSSfxSynthStop. Pixtone::playResampled stays on SDL3_mixer (v1 scope; v2 candidate). Pixtone::setVolume mirrors gain into _sound_fx[].gain_x256 for SDL_DOSSfxSynthStart consumption; mid-play L3 volume updates are no-op (v2 candidate: SDL_DOSSfxSynthSetGain), with a one-shot caveat banner at first L3-active setVolume so operator sees the named cause if a scripted fadeout sounds wrong. Per-slot raw_master_buf is DPMI-locked at Pixtone::_prepareToPlay so the SB16 IRQ-5 ISR mix loop can safely read across IRQ context. Probe-compat: [pixtone-multi] live field + SampleSfxSynthActiveBucket histogram both SUM g_dos_sfx_synth_active_count + g_dos_sfx_synth_pool_active_count so discriminator pillar 2 (max_active_count) holds across L3 OFF/ON paths. Pixtone::stop disambiguates the routing via on_irq_pool flag set at play-time (one bit per stPXSound; 0 = SDL3_mixer path; 1 = L3 ISR-pool path). On a default boot the ENABLED variant is expected (default-ON since v1.0.1); the killswitch SDL_HINT_DOS_SFX_SYNTH_IRQ_MIX=0 disables, and a non-default Organya AUDIO_BACKEND forces the DISABLED-refused variant. See docs/internal/LEVER3-PIXTONE-IRQ-MIX-DESIGN.md sec.4 (engine routing) + sec.10 contract lock)"
  "0168 Lever 2b engine-side routing + DPMI locks + pause-coherent stop/start (MidiScheduler tick from IRQ-5 ISR; engine companion to patches/SDL/0072; optional -- LOG_INFO banner emitted once at SoundManager::init narrating ENABLED (engine registered tick_isr callback (L2b active)) vs DISABLED (engine skipped registration; SDL_HINT_DOS_MIDI_ISR_TICK unset or =0). INFO-level; appears only on tagged runs or DOS_PORT_LOG_VERBOSE=1. The runtime witness that the engine-side ISR-callback registration engaged. On the ENABLED path, MidiScheduler::tick_isr is registered via SDL_DOSMidiTickRegister + invoked from the SB16 IRQ-5 ISR at ~43 Hz Tier-2 with monotonic now_ms; the engine main-loop MidiScheduler::tick call at SoundManager::runFade is SKIPPED to avoid double-dispatch. tick_isr uses fixed-point _us_per_tick_x256 (no FPU per ISR discipline) instead of the engine-thread tick()'s double _us_per_tick. Reads from a DPMI-locked 16K-entry MidiEvent[] buffer (g_midi_isr_events) refreshed from _events at every load/load_from_preload_cache mutation under cli/sti (MidiIsrCriticalSection RAII; gated on SDL_DOSMidiIsrTickActive for L2b-OFF byte-identity). MidiBackendOpl3 instance DPMI-locked at ctor; runtime patch bank (.bss-resident) DPMI-locked at _try_load_opl3_patches_dat success. Pause-behavior choice (b) per operator: SoundManager::pause explicitly calls MidiScheduler::stop under L2b ON + SoundManager::resume re-arms via start -- preserves today's 'music halts when paused' semantic; one-shot banner narrates the explicit-stop engagement on first pause transition. On a default boot the DISABLED variant is expected (lever off); engaging requires SDL_HINT_DOS_MIDI_ISR_TICK=1 + a non-Organya backend (Organya doesn't use MidiScheduler). See docs/internal/LEVER2B-MIDI-ISR-TICK-DESIGN.md sec.1-7 + sec.8 contract lock)"
  ...
  "0213 v1.0.8.1 eager action-sheet load decision (default-ON; LOG_INFO at first load_stage; pre-loads weapon bullet + hit/muzzle/trail caret sheets so the first fire/impact does not lazy-decode mid-gameplay; killswitch SDL_HINT_DOS_EAGER_ACTION_SHEETS=0. OPTIONAL: INFO-level, present on verbose/tagged runs. BANNER_REGEX idx 92.)"
  "0214 v1.0.8.1 organya render-window quiesce decision (default-ON; emits on an organya COLD-RENDER; zeroes the SB16 ring so the DMA loops silence not stale music during the blocking synth pass; killswitch SDL_HINT_DOS_ORG_RENDER_QUIESCE=0. OPTIONAL: cold-render-only, absent in a warm-cache smoke. BANNER_REGEX idx 93.)"
  "0215 v1.0.8.1 batch org-precache total (OPTIONAL -- emits ONLY under DOS_PORT_ORG_PRECACHE_ALL=1, the deployable-cache one-shot; reports rate/channels/rendered/skipped/failed/bytes for CF sizing; absent in a normal gameplay smoke. BANNER_REGEX idx 94.)"
  "0216 DOSKUTSU.CFG config-file setenv shim (REQUIRED -- emits every boot after Logger::init: 'config: loaded DOSKUTSU.CFG (N keys)' when DOSKUTSU.CFG is present in the program dir, else 'config: no DOSKUTSU.CFG, using built-in defaults'. SETUP.EXE writes the file; the shim setenv's each user-facing key BEFORE any getenv/SDL_GetHint read, overwrite=0 so precedence is env > file > built-in default. Two-witness with the consuming lever's own banner -- e.g. a CFG with PERF_MODE=1 yields 'perf-mode: level=1'; a BAT 'SET SDL_HINT_DOS_PERF_MODE=0' over the same CFG yields 'perf-mode: level=0' (env wins). Default smoke stages NO CFG so the 'no DOSKUTSU.CFG' variant matches. BANNER_REGEX idx 95.)"
  "0221 organya auto-precache decision (OPTIONAL -- organya-gated, emits ONLY on an organya run, one of 'ENABLED (default)' / 'DISABLED (killswitch =0)' via SDL_HINT_DOS_ORG_AUTOCACHE; mirrors the 0214 render-quiesce banner. First-launch renders all songs behind a progress overlay + writes the per-tier READY.<sha> sentinel; subsequent boots skip on the sentinel; in-game cold-render overlay armed for stray misses. Absent under the default OPL3/WB backend (no cold-render) which is correct not a failure; the headless DOS_PORT_ORG_PRECACHE_ALL=1 path returns before registration so it is also absent there -- byte-identical to 0215.)"
  "0223 small-polish item-4c DOS resolution-label lock (default-ON; killswitch SDL_HINT_DOS_RES_LABEL_LOCK=0. Emits every DOS boot from Renderer::initVideo: 'Renderer::initVideo: DOS resolution-label lock ENABLED (default)' (or DISABLED (killswitch =0)). The actual behavior -- Video menu shows the locked 320x240 + Resolution scroll is a no-op -- lives in options.cpp and only runs when the menu is opened, so the boot banner is the two-witness runtime side; embed witness = strings|grep 'DOS resolution-label lock'. REQUIRED at the v1.1.2 ship-gate (default-ON, initVideo emits at every boot; g2k-validated). BANNER_REGEX idx 97.)"
  "0224 small-polish item-3 menu slide-in fixed-timestep fix (default-ON; killswitch SDL_HINT_DOS_MENU_SLIDE_FT=0. Forced boot eval in main.cpp after textbox.Init emits 'menu slide-in fixed-timestep: ENABLED (default; ...)' (or DISABLED (killswitch; ...)). Moves the StageSelect WARP-banner (fWarpY) + SaveSelect char-pic (fPicXOffset) slide STEP into the sub-prompt TickState() so it advances at 50 Hz under FT GM_NORMAL instead of render rate; non-FT paths (FT=0/GM_TITLE/inventory) keep stepping in Draw byte-identically. Embed witness = strings|grep 'menu slide-in fixed-timestep'. REQUIRED at the v1.1.2 ship-gate (default-ON, boot-forced eval emits at every boot; g2k-validated). BANNER_REGEX idx 98.)"
  "0226 #39b custom MIDI drop-in dir resolution (OPTIONAL -- emits ONLY when MIDI_SET / SDL_HINT_DOS_AUDIO_MIDI_SOURCE names a path-safe data/<dir>/ holding >=1 .mid that is NOT a known set (wiimidi/orgmid), AND the killswitch SDL_HINT_DOS_AUDIO_MIDI_CUSTOM_DIRS is not '0'. Default smoke uses MIDI_SET=wiimidi so this is ABSENT -- correct, not a failure; embed witness = strings|grep 'custom drop-in dir'. The #39b DOSBox validation cell (MIDI_SET=mycustom + staged data/mycustom/*.mid) is the runtime witness; killswitch=0 falls back to wiimidi with no banner. BANNER_REGEX idx 99.)"
  "0227/0230 Phase-3 input bindings + gameport axis maps (REQUIRED -- '[input-bind] applied=N invert_y=M dos_axes=1' emits every boot from input_apply_cfg_bindings; applied=count of BIND_* overlaid, dos_axes=1 = the DOS axis->action maps re-asserted post settings_load. Embed witness = strings|grep DOS_PORT_BIND_. BANNER_REGEX idx 100.)"
  "SDL/0102 gameport persisted-calibration applied (OPTIONAL -- SDL-log '[joycal] applied stored cal x=[..] y=[..]'; fires ONLY when SDL_HINT_DOS_JOY_CAL is set + an emulated/real gameport is present, so ABSENT in the default no-joystick smoke is correct. Embed witness = strings|grep joycal. BANNER_REGEX idx 101.)"
  "0231 v1.0.9 HQ Organya tier (optional -- emits ONLY when SDL_HINT_DOS_AUDIO_TIER2=0 resolves the 22050 stereo HQ tier; default smoke runs Tier-2 LQ so the line is ABSENT, expected not a failure; the HQ smoke cell sets AUDIO_TIER2=0 to witness it. Embed witness = strings|grep org-hq. BANNER_REGEX idx 102.)"
  ...
  "0234 Campaign 2 AdLib backend selection (OPTIONAL -- 'audio backend: adlib (Campaign 2 -- native OPL2 FM synthesis ...)' emits at selectBackendFromEnv ONLY under SDL_HINT_DOS_AUDIO_BACKEND=adlib. The default smoke leaves AUDIO_BACKEND unset (-> opl3 default) so this is ABSENT, expected, not a failure (same OPTIONAL gating as the wave-44 AUDIO_BACKEND=opl3/wb selection banners). The witness is the dedicated AdLib cell: a no-Sound-Blaster DOSBox config (sbtype=none + oplmode=opl2) with SET SDL_HINT_DOS_AUDIO_BACKEND=adlib. Embed witness = strings|grep 'audio backend: adlib'. BANNER_REGEX idx 108.)"
  ...
  "0235 Campaign 2 AdLib PIT/IRQ-0 OPL music pump started (OPTIONAL -- 'adlib: OPL music pump STARTED at N Hz (PIT ch0 / IRQ-0 drives MidiScheduler::tick_isr; BIOS 18.2 Hz tick chained; restored on exit) ...' emits at SoundManager::init ONLY under AUDIO_BACKEND=adlib AND when SDL_DOSOplTimerPumpStart succeeded (OPL2 detected, SB not hot, hz in [19,1000]). This is THE decisive runtime witness that the no-SB music clock engaged -- on a real AdLib/OPL2 card or PicoGUS /mode adlib the 8253 PIT ch0 is reprogrammed to N Hz (default 120; override SDL_HINT_DOS_OPL_TIMER_HZ) and IRQ-0 drives the SAME tick_isr the SB path drives. ABSENT in the default smoke (no adlib), expected. NB per [[dosbox_not_proxy]] DOSBox-X confirms the banner/boot path but NOT real PIT/IRQ-0 timing -- the g2k AdLib iter is the perf/correctness witness (incl. PIT-restore-on-quit + BIOS-tick correctness). Embed witness = strings|grep 'OPL music pump STARTED'. BANNER_REGEX idx 110.)"
  ...
  "0292 song-change dispatch at INFO (OPTIONAL -- '[music-dispatch] tick=N song=N (prev N)' emitted by SoundManager::music() on every song change, with the logic-tick index. INFO-level, so absent on a plain untagged WARN-level run -- expected, not a failure; tagged runs and DOS_PORT_LOG_VERBOSE=1 log at INFO. Shipped logs previously showed a song LOADING but never its DISPATCH, which is why the Shack-music question and the AUTO-vs-WaveBlaster question both needed the operator's ears; from round 2 they are answerable from the log. Embed witness = strings|grep music-dispatch.)"
  "0294 CPU witness at boot (OPTIONAL -- '[cpu-witness] vendor=.. family/model/stepping .. mhz_est=..' or the DISABLED variant, emitted once after config load. INFO-level, so absent on an untagged WARN run -- expected, not a failure. Nothing in a log previously witnessed WHICH MACHINE ran a cell: video was provable from vram/LFB evidence and sound from is_sb16/dsp_ver, while the CPU rested entirely on a digit typed into a sweep BAT, so every cross-CPU conclusion in the campaign rested on one unverifiable assertion. Reuses the silicon-pinned MHZ_486_LOOP_DIV so engine numbers stay comparable with SETUP's. Default-ON, strict killswitch SDL_HINT_DOS_CPU_WITNESS=0, because a boot-time calibration loop on hardware we do not control needs an escape hatch. Cross-check against the BAT-written .NFO manifest: a disagreement means the wrong sweep argument was passed and every number in that run is mislabelled. Embed witness = strings|grep cpu-witness.)"
  "0296 oversized-surface centring decision (OPTIONAL -- LOG_INFO emitted once in Renderer::initVideo, right after the renderer is created, in one of three variants: 'not needed (surface matches the logical screen)' on every card that can set 320x240 (the DEFAULT smoke case, and the state every banked benchmark cell was measured in), 'ENGAGED' when the backend could only give a larger surface, or 'DISABLED (killswitch =0)' under SDL_HINT_DOS_CENTER_OVERSIZED=0. INFO-level, so absent on a plain untagged WARN-level boot -- expected, not a failure (same gating as [[cpu-witness]] above). The ENGAGED variant is the runtime witness that the ATI Mach64-CT path took the centring route: it reports the surface size and the offset, and is followed by a 'center-oversized: margins cleared' line and, on the first partial flush, a 'center-oversized: flush rect=WxH@X,Y bytes=N (surface would be ...)' line that proves the steady-state flush stays logical-sized rather than surface-sized. Cannot be witnessed on the reference g2k hardware (Cirrus offers 320x240); reproduce under DOSBox-X with [video] allow low resolution vesa modes = false, which forces a 640x480 stand-in for the Mach64's 512x384. Embed witness = strings|grep center-oversized. BANNER_REGEX idx 147.)"
  "0297 oversized-surface fast-path decision (OPTIONAL -- LOG_INFO emitted once in Renderer::initVideo, immediately after the 0296 'center-oversized: ENGAGED' line, and ONLY in that branch: on a card that offers 320x240 the centring block never runs, so this line is absent by design and its absence is not a failure. Two variants: 'ENGAGED' (default) means the hand-rolled blit and fill paths follow the centred box instead of standing down, and 'DISABLED (killswitch =0)' under SDL_HINT_DOS_FASTPATH_OVERSIZED=0 means they stand down while the centring stays -- which is the Round G A/B arm, deliberately a SEPARATE hint from 0296's so the comparison isolates the blit route rather than swapping one picture for another. Runtime confirmation that the patch is doing anything lives in two existing counters, not in this banner: 'fastpath audit ... taken=' must go non-zero on an oversized surface (it read taken=0 skipped=1782 on the round-2 Mach64) and the matching 'slowpath reason block ... windim=' must fall to zero. Cannot be witnessed on g2k (Cirrus offers 320x240); same DOSBox-X stand-in as 0296. Embed witness = strings|grep fastpath-oversized. BANNER_REGEX idx 148.)"
  "0310 off-screen tile-slot skip decision (REQUIRED -- 'tile-bbox-skip: ENGAGED (default since 0310)|DISABLED (killswitch =0) -- off-screen tile slots ...' emitted once in Renderer::initVideo on EVERY boot, on EVERY card. Deliberately OUTSIDE the oversized-centring block: the lever is card-independent, so a Mach64-only witness would be absent on the very cards where the fps was measured. Patch 0113 added the lever with NO banner and default-OFF, and it then sat unmeasured for eleven waves -- Round J finally A/B'd it on the DX2-66 and measured +0.6 fps (per_loop_fps 22.5 -> 23.1, higher on all five Mimiga samples, not just the aggregate), so 0310 flips the default and adds this witness. REQUIRED rather than optional precisely because the absence of a banner is what let 0113 hide: a lever nothing can witness cannot satisfy the two-witness rule, and the same gap forced Round J to INFER TILEMAP_CACHE engagement from a -6 fps delta instead of reading it. Strict-0 killswitch SDL_HINT_DOS_TILE_BBOX_SKIP=0 flips the text to DISABLED and still matches. Embed witness = strings|grep tile-bbox-skip. BANNER_REGEX idx 149.)"
  "0311 window-surface readback probe (OPTIONAL -- 'surface-extent: box=320x240@X,Y pitch=N probes(4x4 ...)=[v,v,...]' emitted once per 100 flips, ONLY under SDL_HINT_DOS_SURFACE_EXTENT=1 AND only when 0296 centring is active. Default OFF and absent on every card that offers 320x240, so absence is correct rather than a failure -- and it cannot fire in the default smoke at all. Exists because FOUR mechanisms for the Mach64 black-right/bottom defect were wrong (0297 gate, 0308 thrash arm, a dirty-rect coordinate theory, the flush-shape theory behind the retracted 0309), six draw/flush paths each read as correctly centred, and the flush rect is witnessed correct at 320x240@96,72 -- while NEITHER emulator can reproduce the defect (DOSBox-X renders the centred path correctly at 640x480; 86Box's Mach64 advertises the double-scan bit real silicon clears, so it offers 320x240 and never centres). Reads the window surface back to say whether missing pixels are absent from the SURFACE or only from the SCREEN: right/bottom probes non-zero means the pixels are present and the defect is downstream in the LFB write or scanout, which every test to date has been blind to; zero with non-zero controls means a draw path is mis-sized after all; controls also zero means the frame was blank and the sample says NOTHING. Samples 16 points rather than scanning because a full 512x384 readback is ~11 ms on this CPU (Round J measured ~17 MB/s flat beyond an 8 KB L1) and would distort the frame it measures. Embed witness = strings|grep surface-extent. BANNER_REGEX idx 150.)"
  "0314 composite world cache stage 3a (REQUIRED -- 'world-cache: ENABLED (opt-in)|DISABLED (default) -- stage 3a composite ...' emitted once on the first DrawScene call of every boot, regardless of killswitch state, so it is deterministic on any boot that reaches gameplay. Variant B: the cache holds the BG+FG TILE layers only and is colorkeyed; the backdrop stays on its own path and shows through, which also removes the BK_PARALLAX problem since a world-origin key is wrong for a parallax backdrop. STAGE 3a IS A CORRECTNESS GATE AND IS EXPECTED TO BE SLOWER WHEN ENABLED -- it rebuilds the composite unconditionally every frame with no key and no possible hit; the saving arrives in 3b with the tile-aligned key. Do NOT bank an fps number from an enabled 3a cell. REQUIRED rather than optional for the reason 0310 gives: a lever nothing can witness cannot satisfy the two-witness rule, and patch 0113 hid for eleven waves behind exactly that gap. Runtime engagement is witnessed by the separate per-100-frame '[world-cache n=.. composites=.. declined=.. bytes=..]' counter line -- REAL counters, unlike the tm_route_cache literal that misled every reader of the Round N logs. Embed witness = strings|grep world-cache. Opt-in SDL_HINT_DOS_WORLD_CACHE=1, default OFF.)"
  "0315 world-cache tile-aligned key stage 3b (OPTIONAL -- 'world-cache-key: ENABLED (opt-in)|DISABLED (default) -- stage 3b tile-aligned key ...'. OPTIONAL and not required, deliberately: the emit sits INSIDE world_cache_draw_tiles() past the SDL_HINT_DOS_WORLD_CACHE early-return, so a default smoke (world cache off) never reaches it and its absence there is correct. A required entry would fail the default gate. The witness is a cell that sets WORLD_CACHE=1. This is the stage where the saving arrives: 3a composited unconditionally, 3b consults a TILE-ALIGNED key so a hit survives 15 of every 16 pixels of camera travel -- the specific fix for patch 0097, which keyed on pixel scroll, missed on every pixel of camera motion, and needed a 0.92 hit rate it never approached. Break-even here is 0.33. Gated separately from the 3a hint so ONE binary carries three arms: neither = uncached, WORLD_CACHE=1 = 3a unconditional composite, WORLD_CACHE=1 + WORLD_CACHE_KEY=1 = 3b keyed. Read the real hit rate from the '[world-cache n=.. hits=.. composites=.. declined=.. hit_pct_x10=..]' line, NOT from this banner. Embed witness = strings|grep world-cache-key.)"
  "0316 whole-tile opaque dword copy (REQUIRED -- 'Renderer::initVideo: tile dword-copy lever ENABLED (opt-in)|DISABLED (default) -- whole-tile opaque dispatch ...' emitted once in initVideo on EVERY boot regardless of killswitch state, so it is deterministic. Fully-opaque 16x16 tiles skip the per-pixel colorkey compare and move as dwords; Round N measured the tilemap colorkey loop at 84.1% opaque pixels and the tile layers at 4.9 MB/s against a 17.0 MB/s memcpy floor. Differs from 0145 (+0.40) by deciding ONCE PER TILE from one bit instead of walking a per-row run list and paying rep-setup per run. No stale-tile class: tile opacity is a property of tile CONTENT, so camera motion cannot change it and a destructible tile changes the tile INDEX not the pixels behind it; only a tilesheet reload invalidates, handled by Surface::cleanup(). Deliberately NOT named OPAQUE_TILE_FASTPATH -- that literal is carried at severity forbidden in this same array, because the wave-38 lever of that name went out in a batch rollback (3-lever stack, -14 fps, cause identified as a different lever, on a build missing patches) and was never judged on merit. Mechanism unmeasured, not refuted; only the name is retired. Engagement is witnessed by the UNGATED per-100-flip '[tile-dword-copy n=100 engaged=.. fallback=..]' line, ungated on purpose so a plain fps cell can prove engagement without instrumentation that perturbs the number it measures -- patch 0113 hid for eleven waves behind exactly that gap. Embed witness = strings|grep tile-dword-copy. Opt-in SDL_HINT_DOS_TILE_DWORD_COPY=1, default OFF.)"
  "0317 native-mode pin killswitch (REQUIRED -- 'pin-native-mode: ENGAGED (default)|STOOD DOWN (DOS_PORT_PIN_NATIVE_MODE=0) ...' emitted once on EVERY boot regardless of killswitch state, so it is deterministic. The pin (patches/SDL/0012) suppresses any VBE mode-set that would change VRAM dimensions after the first. Its rationale claimed hardware without a native 320x240 lands on the 640x480 fallback at mode-set #1 so the pin is harmless there; MEASURED FALSE on the Mach64 215CT under UniVBE 6.70, where closest-match picks 512x384 (id=0x01F3) and the 640x480 request IS suppressed. That matters because 512x384 in that mode runs near 75-77 Hz, outside the 60 Hz lock range of the QA capture hardware, so every measured cell on that card is unobservable -- a wedge is detectable but not diagnosable. There was no runtime path to 640x480 at all: the hint is set with SDL_HINT_OVERRIDE so no env var can beat it, which is why this is a code killswitch rather than a hint. Default UNCHANGED (pin engaged) unless DOS_PORT_PIN_NATIVE_MODE is exactly \"0\". The upper-left-rendering bug the pin guards predates patch 0296 centre-in-oversized, which is correct on this card at 512x384 (ENGAGED offset 96,72, flush 76800 bytes); whether it is correct at 640x480 is what this MEASURES and is not assumed. The test verifies itself -- if the mode is reached, capture locks and the image shows whether rendering is right or the old bug returned. Do NOT compare an unpinned cell against a pinned one as a hardware result: the present path differs (direct-VESA presents full-surface only and stands down on the centred partial path), so record direct_vesa_presents. Embed witness = strings|grep pin-native-mode.)"
  "0318 Round-Q tick-anchored frame dump (env-gated DOS_PORT_SHOT_TICKS, DEFAULT-OFF; optional -- LOG_INFO '[shot-dump] ARMED n=.. first=.. last=..' at the FIRST flip after the var is parsed, then one WROTE/SKIPPED/TRUNCATED line per named tick. Default smoke leaves SHOT_TICKS unset so this is ABSENT, which is expected and not a failure: unset costs one int compare per flip and the binary behaves as pre-patch. Embed witness = strings|grep shot-dump. The runtime witness is any cell that sets DOS_PORT_SHOT_TICKS -- the Round-Q MQ cells and the DOSBox-X reference run both do. Dumps the composed logical 320x240 box to LOGS\\Snnnnn.PPM (binary PPM, palette resolved, fopen wb per the DJGPP rule). Hooked at the top of Renderer::flip() so BOTH run_tick and run_tick_fixed are covered -- the fixed path is the default and is where engine timers have gone blind before. OPTIONAL: diagnostic, default-OFF; absent on release binaries which lack 0318. BANNER_REGEX idx 155.)"
  "0319 Round-Q frame-dump coverage summary (rides DOS_PORT_SHOT_TICKS, DEFAULT-OFF; optional -- one LOG line per armed tick at shutdown, before TAS::shutdown, reporting fired_at=N or NEVER FIRED. This is the FIX for the 0318 defect found on cell GVSD: 0318 tested tick==target once per flip, and with logic ticks at a true 50 Hz against a 29.5 fps flip rate only ~59% of tick values were ever tested, so 2 of 4 requested dumps silently did not happen -- no file, no log line, no failure to report. 0319 fires on the first flip at or AFTER the target (cannot miss) and makes the check state its own coverage, because a silent absence is indistinguishable from never having been asked. Per-dump lines also gain want=/got=/skew=. Default smoke leaves SHOT_TICKS unset so this is ABSENT, expected not a failure. Embed witness = strings|grep 'shot-dump. summary'. OPTIONAL: diagnostic, absent on release binaries which lack 0318/0319. BANNER_REGEX idx 156.)"
  "0320 Round-Q backdrop thrash centring fix (DEFAULT-ON correctness fix, killswitch SDL_HINT_DOS_THRASH_CENTRE=0; optional -- LOG_INFO on the first thrash-branch entry ON AN OVERSIZED SURFACE. Requires BOTH a cache-miss streak AND dosCenterActive(); the && short-circuits, so a 320x240 card never evaluates the lever and never emits, which is CORRECT and expected. Verified under DOSBox-X: thrash engages (streak 184) and this banner is absent because centring is inactive -- which is also the regression witness that the fix is inert at 320x240. Fixes the backdrop-black defect on oversized surfaces: patch 0126 routes the tile loop direct-to-window on miss streaks, using the cache path coordinate convention but skipping the cache->window blit that applies the centring offset (0302), so the backdrop landed at window origin instead of the centred box. Confirmed on hardware Round Q -- BACKDROP_CACHE=0 alone took the void 78006 px -> 0, and 99.95 pct of void pixels obey the predicted 160x120 quadrant boundary. Adds a clip rect so the tile loop cannot over-blit into the cleared margins. NO effect when the surface is exactly 320x240 (dosCenterActive false, offsets zero). Embed witness = strings|grep thrash centring. BANNER_REGEX idx 157.)"
  "patch SDL/0126 force-mode-id filter (SDL_HINT_DOS_FORCE_MODE_ID; REQUIRED -- one banner per process emitted UNCONDITIONALLY at DOSVESA_GetDisplayModes (video init, always reached before the title, same SDL-log channel as the SDL/0123 DACWIDTH required banner), so it fires on every run incl. the default smoke where the hint is UNSET -> the 'hint unset -> filter inactive' variant. Three variants: hint-unset (filter inactive, default smoke), requested-id FOUND (enumeration filtered to that one id, names the resulting WxH pick), or requested-id NOT-enumerated (loud fallback to the unfiltered list -- never an empty mode list). Forces the mode pick past SDL_GetClosestFullscreenDisplayMode's closest-aspect tie-break which excludes 640x400 on the Mach64 card ROM and hands UniVBE runs to 512x384; shared dependency of MACH64-30FPS-PLAN Phase 1 (force 0x0101 under UniVBE) and Phase 2 (force 0x0100). Filters the enumeration list only -- leaves closest-match/pin/LFB-decision/surface-lifecycle untouched. Embed witness = strings|grep DOSVESA-FORCEMODE. BANNER_REGEX idx 158.)"
  "0328 BG blank-tile skip lever (patch 0328; required -- one banner per process in Renderer::initVideo, narrating ENABLED (default) when SDL_HINT_DOS_BG_BLANK_SKIP is unset vs DISABLED (killswitch) when =0; the BG tile loop skips drawing fully-transparent (all-colorkey) tiles, a byte-identical no-op; fires deterministically at video init every boot so present in smoke; default is ON so ENABLED is the expected smoke verdict)"
)
