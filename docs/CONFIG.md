...
| `SDL_HINT_DOSKUTSU_AUDIO_MIDI_SOURCE` | `orgmid2`, `wiimidi`, `<dir>` | `orgmid2` | Which MIDI set the `gus` / `opl3` / `wb` backends play. `orgmid2` (default): our org2mid native-GM conversion (`data/orgmid2/`; generate with `make convert-music`), which auto-falls-back to `wiimidi` if that dir is absent. `wiimidi`: the WiiWare arrangement (`data/midi/`). `orgmid`: the Hart legacy `.mid` set (`data/orgmid/`). Any other value is treated as a custom drop-in directory name: if `data/<dir>/` holds your own `.mid` tracks, the engine plays from there (see "Bring your own MIDI set" in `docs/ASSETS.md`). SETUP exposes this as the **MIDI music set** row (writing the `MIDI_SET` DOSKUTSU.CFG key) -- known sets show as WiiWare / OrgMIDI, drop-ins as `Custom (<dir>)`. | None |
...
shared with the music; SETUP enables them by default when you pick Gravis (choose
