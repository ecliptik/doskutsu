...
| `SDL_HINT_DOSKUTSU_GUS_SFX_VOICES` | `0`..`voices/4` | `5` | GF1 voices reserved for sound effects under `AUDIO_BACKEND=gus` (music-only backends ignore it). The GF1's hardware voices are shared between music and effects; this reserves a slice for effects so a burst of SFX cannot steal voices from the music (which otherwise cuts its own oldest notes -- heard as "scratchy" / thinned music). At the default 20 voices this splits **15 music + 5 SFX**. Clamped to `voices/4` so music always keeps at least three-quarters of the pool; `0` = no reservation (effects share the whole pool, the pre-tuning behavior). | Leave at `5` (the g2k ear-judged best). Effects share the GF1 voices, so pairing this with `pgusinit /gus44k 1` + 32 voices gives both the most room. |
| `SDL_HINT_DOSKUTSU_GUS_SFX_GAIN` | `0`..`150` | `70` | Sound-effects loudness (percent) under `AUDIO_BACKEND=gus` (music-only backends ignore it). Scales the GF1 effect-voice volume so effects sit **under** the velocity/CC-scaled music instead of over it. `100` = the raw effect volume (pre-tuning -- effects come out louder than the music); `70` (default) balances them; `0` silences effects; values up to `150` boost. | Leave at `70`; lower it if effects still feel too loud |
...
| `SDL_HINT_DOSKUTSU_SFX_DEVICE` | `sb`, `none` | `sb` | Which device plays the sound effects (the SETUP "Select Sound FX Device" selection). `sb` (default): the Sound Blaster DAC. `none` = **No Sound FX** (sound effects off; music still plays). Under `AUDIO_BACKEND=gus` the effects play on the **GF1 wavetable** and are on by default (leave `SFX_DEVICE` unset -- the engine derives the native GF1 device); set `none` for music only. | None |
...
| `SDL_HINT_DOSKUTSU_AUDIO_MIDI_SOURCE` | `orgmid2`, `wiimidi`, `<dir>` | `orgmid2` | Which MIDI set the `gus` / `opl3` / `wb` backends play. `orgmid2` (default): our org2mid native-GM conversion (`data/orgmid2/`; generate with `make convert-music`), which auto-falls-back to `wiimidi` if that dir is absent. `wiimidi`: the WiiWare arrangement (`data/midi/`). `orgmid`: the Hart legacy `.mid` set (`data/orgmid/`). Any other value is treated as a custom drop-in directory name: if `data/<dir>/` holds user-supplied `.mid` tracks, the engine plays from there (see "Custom MIDI set" in `docs/ASSETS.md`). SETUP exposes this as the **MIDI music set** row (writing the `MIDI_SET` DOSKUTSU.CFG key) -- known sets show as WiiWare / OrgMIDI, drop-ins as `Custom (<dir>)`. | None |
...
SET SDL_HINT_DOSKUTSU_AUDIO_MIDI_SOURCE=mymidi   REM play custom data/mymidi/*.mid (see docs/ASSETS.md)
```
