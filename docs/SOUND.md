...
- **OrgMIDI** (`orgmid`) -- a note-for-note transcription of the original
  Organya music (`data/orgmid/`), generated by `make convert-music`. It maps the
  Organya drums to standard General MIDI percussion keys.

On the **Gravis UltraSound**, the two sets voice their drums differently. The
WiiWare set uses some non-standard percussion notes outside the GM 27-87 range;
the GUS backend maps those to the nearest defined percussion so nothing drops
silent, but if a track's drums sound off under **WiiWare**, try **OrgMIDI** --
its drums land on the exact GM keys the patch set expects. (The other backends
are unaffected.)
